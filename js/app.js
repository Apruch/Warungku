...
// ── ALIAS FUNGSI YANG HILANG ──────────────────────────────────
function doDataPengguna(){ doSignupFinish(); }
function doLupaSandi(){ lupaSandiCariAkun(); }
function resetLupaSandiForm(){ if(typeof _lupaTampilStep==='function') _lupaTampilStep(1); }

var _filterState = { kat:'Semua', stok:'Semua', sort:'Nama A-Z' };
function toggleFilter(el,group,val){
  _filterState[group]=val;
  var parent=el.parentNode;
  if(parent) parent.querySelectorAll('button').forEach(function(b){b.classList.remove('on');});
  el.classList.add('on');
}
function applyFilters(){
  DB.filterState=DB.filterState||{};
  DB.filterState.kategori=(_filterState.kat&&_filterState.kat!=='Semua')?[_filterState.kat]:[];
  DB.filterState.sortStok=_filterState.stok==='Menipis'?'Hampir Habis':_filterState.stok==='Habis'?'Tersedikit':'';
  DB.filterState.sortHarga=_filterState.sort==='Harga Tertinggi'?'Termahal':_filterState.sort==='Harga Terendah'?'Termurah':'';
  katalogPage=1; go('pg-katalog');
}
function resetFilters(){
  _filterState={kat:'Semua',stok:'Semua',sort:'Nama A-Z'};
  DB.filterState={kategori:[],sortStok:'',sortHarga:''};
  katalogPage=1; go('pg-katalog');
}
}
...
function resetLupaSandiForm(){ if(typeof _lupaTampilStep==='function') _lupaTampilStep(1); }
var _filterState={kat:'Semua',stok:'Semua',sort:'Nama A-Z'};
function toggleFilter(el,group,val){_filterState[group]=val;var p=el.parentNode;if(p)p.querySelectorAll('button').forEach(function(b){b.classList.remove('on');});el.classList.add('on');}
function applyFilters(){DB.filterState=DB.filterState||{};DB.filterState.kategori=(_filterState.kat&&_filterState.kat!=='Semua')?[_filterState.kat]:[];DB.filterState.sortStok=_filterState.stok==='Menipis'?'Hampir Habis':_filterState.stok==='Habis'?'Tersedikit':'';DB.filterState.sortHarga=_filterState.sort==='Harga Tertinggi'?'Termahal':_filterState.sort==='Harga Terendah'?'Termurah':'';katalogPage=1;go('pg-katalog');}
function resetFilters(){_filterState={kat:'Semua',stok:'Semua',sort:'Nama A-Z'};DB.filterState={kategori:[],sortStok:'',sortHarga:''};katalogPage=1;go('pg-katalog');}
>>>>>>> a4646838caf0f1d756d9ca571b28165d2d69e25d
